trigger EventSpeakerTrigger on EventSpeakers__c (before insert, before update ) {
    ...
    //Iterated with trigger.new because whenever we updating or creating new records we always get Trigger.new which will give the updated records.
    ...
    // Step 2 Here we are quering on event object because we have have to get the start date time of that event and then put in the Map.
    // We can say that for this event this is the start date time.
    ...
    for( EventSpeakers__c es : Trigger.New ){ 
        ...
        for(EventSpeakers__c es1 : relatedEventSpeakerList) {
            if(es1.Speaker__c == es.Speaker__c && es1.Event__r.Start_DateTime__c == bookingTime ){
                es.Speaker__c.addError('The Selected speaker is already booked at that time');
                es.addError('Selected Speaker is already booked at that time');
            }
        }
